...
Usage: please specify 2 arguments - the input fasta file and output directory - as follows:
 docker run -it  -v /path/to/input/fasta:${DEEPLASMID_HOME}/in.fasta  -v  /path/to/output/directory:${DEEPLASMID_HOME}/outdir   billandreo/deeplasmid     deeplasmid.sh  in.fasta outdir
...
if [ $# -ne 2 ]; then
    echo "Usage of Deeplasmid from the Docker image, please specify 2 arguments (input fasta file and output directory): docker run -it  -v /path/to/input/fasta:${DEEPLASMID_HOME}/in.fasta  -v  /path/to/output/directory:${DEEPLASMID_HOME}/outdir   billandreo/deeplasmid     deeplasmid.sh  in.fasta outdir"
    exit 1
fi
